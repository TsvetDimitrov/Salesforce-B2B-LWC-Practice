...
export default class PdfGenerationDemo extends LightningElement {
    ...
    get totalAmount() {
        return this.services.reduce((acc, currectValue) => {return acc = acc + currectValue.amount}, 0)
    }
    ...
    pdfHandler() {
         ...
         generatePDF({recordId: this.recordId, htmlData: content.outerHTML})
            .then(result => {
                console.log(result);
                window.open(`https://tsetso-dev-ed.trailblaze.file.force.com/servlet/servlet.FileDownload?file=${result.Id}`);
